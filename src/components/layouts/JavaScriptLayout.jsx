...
const JavaScriptLayout = () => {
  ...
  // Get JavaScript section from routes
  const programmationSection = routesConfig.find(route => route.path === '/programmation');
  const javascriptSection = programmationSection?.children.find(child => child.path === 'javascript');
  const lessons = javascriptSection?.children || [];

  const courseTitle = 'Cours JavaScript';
  const courseDescription = 'Maîtrisez les fondamentaux de JavaScript';
        ...
        <div className="p-3 sm:p-4 overflow-y-auto h-full">
          ...
          <ul className="space-y-1 sm:space-y-2">
            {lessons.map((lesson, index) => {
              const lessonPath = lesson.path === '' ? '/programmation/javascript' : `/programmation/javascript/${lesson.path}`;
              return (
                <li key={index}>
                  <Link
                    to={lessonPath}
                  ...
                  >
                    <div className="flex items-center">
                      ...
                      {lesson.icon && (
                        <span className="text-lg mr-2 sm:mr-3">{lesson.icon}</span>
                      )}
                      <span className="flex-1 text-sm sm:text-base">{lesson.name}</span>
                    </div>
                  </Link>
                </li>
              );
